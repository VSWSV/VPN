...
for config in "${config_files[@]}"; do
  ...
  [[ $config == ../* ]] && config_path="/root/${config#../}"
...
for config in "${config_files[@]}"; do
  ...
  [[ $config == ../* ]] && restore_path="/root/${config#../}"
...
if [ ${#all_backups[@]} -gt 1 ]; then
  ...
  for ((i=0; i<${#all_backups[@]}; i++)); do
    ...
    [[ $i -eq 0 ]] && echo -e "${green}  [$i] ${all_backups[$i]} (最新, $size)${reset}" || echo -e "${yellow}  [$i] ${all_backups[$i]} ($size)${reset}"
  done
  ...
  if [[ "$choice" != "n" ]]; then
    ...
    deleted_count=0
    for c in "${choices[@]}"; do
      if [[ "$c" =~ ^[0-9]+$ && "$c" -ne 0 && "$c" -lt ${#all_backups[@]} ]]; then
        rm -rf "${all_backups[$c]}"
        success "已删除备份: ${all_backups[$c]}"
        ((deleted_count++))
      fi
    done
    [ "$deleted_count" -gt 0 ] && success "共删除 $deleted_count 个旧备份"
  else
    info "保留所有备份"
  fi
else
...
echo -e "${cyan}╠═════════════════════════════════════════════════════════════════════════════════╣${reset}"
info "🎉 升级完成"
echo -e "${yellow}📌 如需生效，请重启服务:${reset}"
