...
function create_account() {
    ...
    if [[ -z "$DOMAIN_ID" ]]; then
        error_exit "未找到域名 ${DOMAIN}，请先在数据库中添加！"
    fi

    ENCRYPT_PASS=$(doveadm pw -s MD5-CRYPT -p "$PASSWORD")
    MAILDIR="${DOMAIN}/${USERNAME}/"

    mysql -u${DBUSER} -p${DBPASS} -e "INSERT INTO ${DBNAME}.mailbox (domain_id, username, password, name, maildir, active) VALUES (${DOMAIN_ID}, '${USERNAME}', '${ENCRYPT_PASS}', '${NAME}', '${MAILDIR}', 1);"
...
function delete_account() {
    ...
    ID=$(mysql -u${DBUSER} -p${DBPASS} -Nse "SELECT m.id FROM ${DBNAME}.mailbox m JOIN ${DBNAME}.domain d ON m.domain_id=d.id WHERE m.username='${USERNAME}' AND d.name='${DOMAIN}';")
    if [[ -z "$ID" ]]; then
...
function list_accounts() {
    ...
    mysql -u${DBUSER} -p${DBPASS} -e "SELECT CONCAT(username, '@', d.name) AS 邮箱地址, name AS 用户名 FROM ${DBNAME}.mailbox m JOIN ${DBNAME}.domain d ON m.domain_id=d.id WHERE m.active=1;"
    draw_line
}
