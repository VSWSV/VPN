...
# 正确的返回函数（回本菜单）
return_menu() {
  read -p "$(echo -e "💬 ${cyan}按回车键返回数据库管理器菜单...${reset}")" dummy
}
...
while true; do
  ...
  echo -e "  ${yellow}④${reset} ${green}修改root密码${reset}           ${yellow}⑤${reset} ${green}调整数据库容量${reset}"
  ...
  case $opt in
    1)
      ...
      read dbuser
      if [[ "$dbuser" == "root" ]]; then
        echo -e "${red}❌ 不允许使用root作为普通账户！${reset}"
        return_menu
        continue
      fi
      ...
      echo -e "ℹ️ 正在创建数据库和用户，请输入root密码..."
      mysql -u root -p <<EOF
...
EOF
    ...
    5)
      clear
      draw_header
      echo -e "ℹ️ 快捷调整数据库容量（逻辑提醒，非强制限制）"
      echo -e "ℹ️ 请输入要设置提醒容量（单位MB，例如500）："
      read capacity
      echo -e "${yellow}⚠️ 注意：MariaDB本身不支持硬限制，请管理员配合监控！${reset}"
      echo -e "${green}✔️ 容量提醒设置为 ${capacity} MB（仅逻辑提醒）${reset}"
      draw_footer
      return_menu
      ;;
    0)
